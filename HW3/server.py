...
#coding=utf-8
# version=2020/12/16
# select tcp server and multiservice
import sys # argv
import socket, select # socket
...
## struct structure
class chat_struct:
	#__slots__ = [name, addr, port, status]
...
## the method to deal with the command during the chatroom
def chat_method(conn, data):
...
def udp_method( data, if_login, user):
	## hw1 command (register and whoami)
	if "register" in data:
		data = data.split()
		if len(data)!=4: 
	...
	## hw3 command
	elif data == "list-chatroom":
		if if_login == False:
			return "Please login first"
		else:
			msg = "%-20s %-10s"%("Chatroom_name", "Status")
			for chat in chatroom:
				msg += "\n%-20s %-10s"%(chat.name, chat.status)
			return msg

## the method to deal with the command what clients sent to server
def tcp_method(conn, addr, data, if_login, user):
	...
	if data == "exit":
		...
		if if_login == False:
			...
			for chat in chatroom:
				if user == chat.name and chat.status == "open":
	...
	## hw3 command
		...
		if if_login == False:
			...
			try:
				# store <username>, addr, and port, status
				tmp = chat_struct(name=user, addr=addr[0], port=int(data[1]), status="open")	
				chatroom.append(tmp)
				return "start to create chatroom..."
			except:
				return "create-chatroom <port> "
	elif "join-chatroom" in data:
		data = data.split()
		if if_login == False:
			return "Please login first."
		else:
			try:
				for chat in chatroom:
					if chat.name == data[1]:
						return "Action: connection to chatroom server.addr%sport%d"%(chat.addr, chat.port)
				return "The chatroom does not exist or the chatroom is close."
			except:
				return "join-chatroom <owner>"
	elif "restart-chatroom" == data:
		if if_login == False:
			return "Please login first."
		else:
			for chat in chatroom:
	...
	while inputs:
		...
		for s in readable:
			if s is server:
				...
				# udp
				if data == "udp":
					u_data, u_addr = u_server.recvfrom(1024)
					message = udp_method( str(u_data, encoding='utf-8'), if_login, user)
					# print("UDP recv: %s" % str(u_data, encoding='utf-8'))
					u_server.sendto(message.encode(), u_addr)
				
				# tcp 
				elif data:
					message = tcp_method(s, addr, data, if_login, user)
					if message == "exit":
						if s in outputs:
							outputs.remove(s)
						inputs.remove(s)
